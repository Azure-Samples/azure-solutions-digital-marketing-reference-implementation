...
added in advance. For instance, you can define a policy that enables any user who can
...
but it also supports two-factor authentication and smartcard-based login,
or it can defer to external authorities such as the Microsoft Account
infrastructure. You can also optionally define groups of users. A tenant also
defines one or more **applications** that are allowed to use the tenant to log users
...
applications that you host in Azure, but you might not have a choice. If the tenant
...
the set of users who will be using some application you're hosting in Azure. It
...
4.  In the **NEW** panel that opens, the first two columns will already have
     **APP SERVICES** and **ACTIVE DIRECTORY** selected for you. In the third column,
    click **DIRECTORY**.
...
7.  The Azure Management site will show the quickstart page for the tenant. You don't
    ...
    This will show a list of all the users in the tenant; currently, it is just you. If you
    wanted to grant other users access to the management site, you'd do so by going to
    this page and then clicking the **ADD USER** button at the bottom of the page.
    However, you don't need to do that right now, but feel free to if you'd like.
...
1.  Assuming you have Visual Studio 2015 still open from the last lab, return to Visual Studio, 
     right-click on the **AzureKit.Management** project, and select
    **Publish**. Use the **Prev** or **Next** buttons to move to the **Settings** page.
    ...
    creating the tenant, and add ".onmicrosoft.com" to the end.
...
3.  Once Visual Studio has finished deploying your app, it will open it in a web
    ...
    ask you to log in. It will instead show a series of links for managing various aspects
    of the site.
...
use the Azure AD directory you created, it does hide some important details. It's
 ...
 * Created a new Application in the Azure AD tenant, configured to enable the
...
3.  In the **What do you want to do** page that opens, select **Add an application
    ...
    Leave the radio button on **WEB APPLICATION AND/OR WEB API** and click
...
4.  Next, Azure wants you to enter a **SIGN-ON URL** and an **APP ID URL**.
    ...
    seconds creating the app, and then will show you the quick start page for the app.
...
7.  You need one last piece of information: the tenant ID. In addition to a unique
    domain name, all Azure AD tenants have a unique identifier (a GUID). Tenants 
    can have multiple domain names, and you can change the domains
    ...
    your tenant id. Alternately, you can click the **VIEW ENDPOINTS** button at
    ...
    that follows the hostname. It will be the same GUID in all cases; this is
    also your tenant ID.
...
10. Once Visual Studio has finished deploying your app, it will open it in a web
    ...
    The main page of the Azure Kit management site will show. This time it won't
...
for the management site: it shouldn't be open to anyone who discovers the URL. However,
...
and can be used entirely anonymously. However, in one of the later labs 
(the [SQL Server](Lab5-SqlServer.md "SQL Server lab") lab), you will add a feature 
in which the user can opt into (and opt out of) receiving email notifications 
...
if you enable this. Users will be able to enter an email address and AAD will send
...
1.  Go to the Azure management site (not the portal) at
    [https://manage.windowsazure.com/](https://manage.windowsazure.com/). As
...
4.  An **Add directory** dialog will appear. As before, enter a
    ...
    **Note:** You will need the domain name later, so take a note
    of the value you entered.

    Click the tick at the bottom right when you have filled everything in. Wait until
    the tenant has been created. You can watch the progress information at the bottom of the
    screen.
...
5.  Once the directory is ready, it will appear in your list of tenants. Strangely,
    ...
    **Note:** Even if you have the portal open in a browser, it is worth opening a new
    tab. As you will see, it is tricky to navigate between the pages for
...
8.  In the filter textbox that appears, type **B2C**. You should then see
...
11. Back in the **Add social identity provider** blade, click **Set up this identity provider**. 
    This will show a blade asking for the **Client ID** and
...
13. In the page that opens representing the new application, you will see some details.
    ...
    **Properties** section and paste it into the **Client ID** field in the browser
    tab in which you're setting up your B2C tenant. Then, in the **Application Secrets**
    ...
    it the URL that Azure AD will supply as a redirect URL during login. This URL
    ...
    **Platforms** in the Microsoft Account application page:
    ...
    Click **Add Url**. At the bottom of the page, click **Save**.
...
15. In the **Add policy** dialog, set the name to **SignUpOrIn**. Click on **Identity
    providers** and check the **MSA** checkbox.
...
1.  Still in the Azure portal on the B2C settings, on the **Settings** blade, 
    ...
    **Include web app/web API**, click **Yes**. Leave **Allow implicit flow** at **Yes**.
    In the **Reply URL**, enter the URL for your Azure Kit main site, specifying "https."
...
4.  When the web site opens in a browser, click the **Sign in** link at the top right. You
    ...
    is that Azure AD remembers your identity and has not asked you to sign in again. To see 
    the whole login flow, try opening a browser in a "private" mode (e.g., if you're using
    ...
    Log in with your Microsoft Account. Once you have, you will then be redirected back
    to the Azure Kit site as an authenticated user.

## Part 6: Configure the Mobile API to use your B2C Tenant

You will need to create another application for the mobile API; in B2C tenants, 
...
1.  In the **Settings** blade select **Applications**. In the blade that opens, click **+ Add**.
    ...
    **Include web app/web API**, click **Yes**. Leave **Allow implicit flow** at **Yes**.
    ...
    and add ".auth/login/aad/callback" to the end.
...
2.  While the main site happens to use the ASP.NET's support for working with AAD, the mobile
    site is using the Azure Mobile Services server SDK. When using that, we don't normally
    write our own code for authentication. Instead, we're going to configure the Azure App Service
    to authenticate for us.

    Back in the browser tab that has the Azure portal open with the tenant for your
    subscription selected (the one in which you can see all your Web Apps, not the
...
3.  To test this, open the **AzureKit - Mobile UWP Only** solution in Visual Studio. Right-click
    ...
    **Debug | Start Debugging** menu item to run the app.
    ...
    run this app, you might not, in which case you should click the cog icon to show the settings.)

    Under **Web/Mobile Address**, there is a textbox to enter the URL of your API web site,
    specifying "https." Click the save icon at the top right. A progress UI will appear.
    You will then see an error reporting an **Internal Server Error**. This is because you
    ...
    **Connecting to a service** appear. You may see a progress animation, but then you
    ...
    configured authentication. (Since there is more work to do on the backend, there's nothing
